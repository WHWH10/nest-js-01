...
export class HeartrateService {
  ...
  async getHeartRateUserID(userID: string): Promise<Response> {
    const heartRate = await this.heartRateRepository.getCreateHeartRateByUserID(
      userID,
    );
    if (heartRate.length == 0) {
      return new ResponseMessage().success().body('No HeartRate').build();
    }

    return new ResponseMessage().success().body(heartRate).build();
  ...
  async getHeartRateByObjectId(
    objectId: MongooseSchema.Types.ObjectId,
  ): Promise<Response> {
    const heartRate = await this.heartRateRepository.getUserbyObjectId(
      objectId,
    );
    if (!heartRate) {
